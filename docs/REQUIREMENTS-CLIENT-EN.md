...
| Total contract hours | **100 hours** |
| Fixed contract price | **USD 10,000** (= 100 × $100) |
...
**Phase effort**: **65 hours** (**USD 6,500**)

### 3.1 Feature modules

| ID | Module | Description | Acceptance criteria | Hours |
|----|--------|-------------|---------------------|-------|
| **M1** | FDA recall data | Full and incremental sync from public sources; store class, reason, firm, dates, lot info, etc. | Data queryable; last sync time shown | 10 |
| **M2** | Drug directory | U.S. NDC directory; name search to assist manual entry | Search when adding to cabinet | 5 |
| **M3** | Recall matching engine | Match by drug name / manufacturer / NDC; lot comparison; checks on new recalls and newly added drugs; deduplicated alerts | Test cases trigger without duplicate alerts | 12 |
| **M4** | User accounts | Email sign-up/login; password reset; **Google OAuth** | End-to-end flows work | 7 |
| **M5** | Personal medicine cabinet | **Manual entry**; **required**: drug name, manufacturer; **optional**: NDC, lot; edit / deactivate / delete | Cabinet CRUD works | 9 |
| **M6** | Monitoring policy | Monitor until **expected stop-date** (see Section 6) | Per agreed policy | 2 |
| **M7** | Email notifications | Drug name, manufacturer, registration time, class, reason summary, FDA link | Test inbox receives correct emails | 5 |
| **M8** | In-app notification center | History and read/unread state | Consistent with email triggers | 3 |
| **M9** | Class-based copy | Class I / II / III templates (see Section 6) | Templates live after client approval | 2 |
| **M10** | User interface | Cabinet, affected recalls, details, disclaimer | User sees open items | 6 |
| **M11** | Instant recall lookup | **Manual** drug info / NDC; three outcomes (match / possible / none) | Sample cases pass | 5 |
...
| **M14** | Test & launch | Testing, deployment, one UAT round | MVP checklist passed | 4 |
| **M15** | Project management & docs | Demos, acceptance materials, brief English user guide | MVP milestone acknowledged | 3 |
| | **Phase 1 total** | | | **65** |
...
**Phase effort**: **35 hours** (**USD 3,500**)

### 4.1 Feature modules

| ID | Module | Description | Acceptance criteria | Hours |
|----|--------|-------------|---------------------|-------|
| **V2-4** | Family member cabinets | Separate cabinets and alerts per member; all drugs **added manually** | Member switching works | 6 |
| **V2-6** | Lot parsing enhancement | Better lot extraction from recall text (with manually entered lots) | Agreed samples pass | 6 |
| **V2-7** | Recall notice browser | Browse by class and date | Filter and view details | 6 |
| **V2-8** | Notification preferences | Email/SMS toggles, class levels, etc. | Settings match delivery behavior | 4 |
| **V2-9** | SMS notifications (Web) | Opt-in phone binding; recall SMS | Test number receives messages | 6 |
| **V2-10** | Account data export | Export personal data | Download in agreed format | 2 |
| **V2-11** | Test & final acceptance | Full-release UAT and production launch | Final checklist passed | 5 |
| | **Phase 2 total** | | | **35** |
...
| 11 | Contract scope | ☐ Confirm **Web only, manual entry, USD 10,000 / 100 hours** |
...
|-------|-------|-------|--------------|-----------|
| Phase 1 | MVP (manual entry + core alerts) | 65 | 6,500 | |
| Phase 2 | Full Web (SMS, family cabinet, etc.; still manual entry) | 35 | 3,500 | |
| **Total** | **Web product** | **100** | **10,000** | |

- Rate: **USD 100 / hour**  
- Fixed price: **USD 10,000** (scope per this document; changes subject to separate agreement)  
...
|-----------|-------------|-------------|-----------------|
| Contract signed + advance paid | Signed scope + project plan | 30% | 3,000 |
| **Phase 1 MVP acceptance** | Section 3 checklist passed | 40% | 4,000 |
| **Phase 2 final acceptance** | Section 4 checklist passed; full Web live | 30% | 3,000 |
...
2. This contract is **100 hours** of Web work at **USD 10,000**; drug intake is **manual entry** only—no scanning or photo recognition unless agreed in writing with adjusted pricing.  
